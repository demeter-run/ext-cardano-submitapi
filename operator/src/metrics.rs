...
use crate::{get_config, Error, State, SubmitApiPort};
...
impl Default for Metrics {
    fn default() -> Self {
        ...
        let reconcile_failures = IntCounterVec::new(
            opts!(
                "submitapi_operator_crd_reconciliation_errors_total",
        ...
        let metrics_failures = IntCounterVec::new(
            opts!(
                "submitapi_operator_metrics_errors_total",
...
pub fn run_metrics_collector(state: Arc<State>) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            let query = format!(
                "sum by (consumer, network, resource_name, tier) (increase(submitapi_proxy_http_total_request{{status_code!~\"401|429|503\"}}[{start}s] @ {}))",
